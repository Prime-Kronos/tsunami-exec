...
-- ФИКСЫ: Bypass v3, Trol рабочий, UI 480px без чёрного низа, Noclip v2, CoinFarm v3 (fireclickdetector+touch), Scroll Touch
...
task.spawn(function()
    while true do
        ...
        -- МЕТОД 1: телепорт к монете + fireclickdetector + firetouchinterest
        ...
        -- МЕТОД 2: бежим к монете (MoveTo) если ещё не подобрали
        if target and target.Parent then
            ...
            repeat
                ...
                -- Повторяем триггеры каждые 0.3 сек
                if tick()-t0 > 0.3 then
                    t0=tick()
                    tryFireClick(target)
                    tryTouchCoin(target, hrp)
...
mkButton("Misc","📋 Версия (уведомление)",DIM,function()
    pcall(function() game:GetService("StarterGui"):SetCore("SendNotification",{Title="Primejtsu Hub",Text="v5.9 | @Primejtsu | Nazar513000",Duration=4}) end)
end)
...
print("[Primejtsu Hub v5.9] @Primejtsu | Nazar513000 | CoinFarm v3 fireclickdetector+touch 😈🪐")
