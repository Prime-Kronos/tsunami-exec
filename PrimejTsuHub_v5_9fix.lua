...
RunService.Heartbeat:Connect(function()
    local h=getHum() if not h then return end
    if CFG.coinFarm then return end -- фарм сам управляет скоростью
...
RunService.Stepped:Connect(function()
    ...
    elseif noclipWasOn then
        ...
        pcall(function()
            local c=getChar() if not c then return end
            for _,p in ipairs(c:GetDescendants()) do
                -- HumanoidRootPart не трогаем — он вызывает застревание
...
-- Основной цикл — движение к монетам
task.spawn(function()
    while true do
        task.wait(0.05)
        ...
        if bestDist<5 then
            collectCoin(target,hrp) task.wait(0.04) continue
        end

        -- Далеко >50 — телепорт в 2 шага (антикик)
        if bestDist>50 then
            pcall(function()
        ...
        elseif bestDist>15 then
            ...
            repeat
                ...
                -- Обновляем MoveTo чтобы не застрял
                pcall(function() hum:MoveTo(target.Position) end)
...
task.spawn(function()
    ...
    task.wait(0.8) Splash:Destroy()

    -- ═══════════════════════════════════════════
    -- ЭКРАН ПАРОЛЯ — в стиле заставки
    -- ═══════════════════════════════════════════
    local PassScreen=Instance.new("Frame",sg)
    PassScreen.Size=UDim2.new(1,0,1,0) PassScreen.BackgroundColor3=Color3.fromRGB(2,2,8) PassScreen.BorderSizePixel=0 PassScreen.ZIndex=200

    -- Фон-градиент
    local passBG=Instance.new("UIGradient",PassScreen)
    passBG.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(2,2,15)),ColorSequenceKeypoint.new(1,Color3.fromRGB(5,3,20))}) passBG.Rotation=45

    -- Звёзды фона
    math.randomseed(99)
    for i=1,60 do
        local s=Instance.new("Frame",PassScreen) local sz=math.random(1,2)
        s.Size=UDim2.new(0,sz,0,sz) s.Position=UDim2.new(math.random(0,100)/100,0,math.random(0,100)/100,0)
        s.BackgroundColor3=Color3.fromRGB(180,180,255) s.BorderSizePixel=0 s.ZIndex=201 s.BackgroundTransparency=math.random(30,70)/100
        Instance.new("UICorner",s).CornerRadius=UDim.new(1,0)
    end

    -- Иконка ключа сверху
    local keyIcon=Instance.new("TextLabel",PassScreen)
    keyIcon.Size=UDim2.new(0,80,0,80) keyIcon.Position=UDim2.new(0.5,-40,0.18,-40)
    keyIcon.BackgroundTransparency=1 keyIcon.Text="🔑" keyIcon.TextSize=60
    keyIcon.Font=Enum.Font.GothamBlack keyIcon.TextTransparency=1 keyIcon.ZIndex=210

    -- Заголовок
    local passTitle=Instance.new("TextLabel",PassScreen)
    passTitle.Size=UDim2.new(0.8,0,0,40) passTitle.Position=UDim2.new(0.1,0,0.35,0)
    passTitle.BackgroundTransparency=1 passTitle.Text="ДОСТУП ЗАКРЫТ" passTitle.TextColor3=RED
    passTitle.Font=Enum.Font.GothamBlack passTitle.TextSize=24 passTitle.TextTransparency=1 passTitle.ZIndex=210

    -- Подзаголовок
    local passSub=Instance.new("TextLabel",PassScreen)
    passSub.Size=UDim2.new(0.8,0,0,20) passSub.Position=UDim2.new(0.1,0,0.47,0)
    passSub.BackgroundTransparency=1 passSub.Text="Введите пароль для входа в Primejtsu Hub"
    passSub.TextColor3=Color3.fromRGB(140,140,160) passSub.Font=Enum.Font.Code passSub.TextSize=11 passSub.TextTransparency=1 passSub.ZIndex=210

    -- Рамка поля ввода
    local inputBox=Instance.new("Frame",PassScreen)
    inputBox.Size=UDim2.new(0.7,0,0,44) inputBox.Position=UDim2.new(0.15,0,0.55,0)
    inputBox.BackgroundColor3=Color3.fromRGB(15,15,20) inputBox.BorderSizePixel=0 inputBox.BackgroundTransparency=1 inputBox.ZIndex=210
    Instance.new("UICorner",inputBox).CornerRadius=UDim.new(0,10)
    Instance.new("UIStroke",inputBox).Color=Color3.fromRGB(80,80,100)

    -- TextBox внутри
    local tbox=Instance.new("TextBox",inputBox)
    tbox.Size=UDim2.new(1,0,1,0) tbox.BackgroundTransparency=1 tbox.BorderSizePixel=0
    tbox.PlaceholderText="🔒  пароль..." tbox.Text="" tbox.TextColor3=WHITE
    tbox.PlaceholderColor3=Color3.fromRGB(80,80,100) tbox.Font=Enum.Font.GothamBold tbox.TextSize=16
    tbox.ClearTextOnFocus=true tbox.ZIndex=211

    -- Кнопка ВОЙТИ
    local enterBtn=Instance.new("TextButton",PassScreen)
    enterBtn.Size=UDim2.new(0.5,0,0,40) enterBtn.Position=UDim2.new(0.25,0,0.67,0)
    enterBtn.BackgroundColor3=Color3.fromRGB(25,25,35) enterBtn.BorderSizePixel=0 enterBtn.BackgroundTransparency=1
    enterBtn.Text="ВОЙТИ  →" enterBtn.TextColor3=RED enterBtn.Font=Enum.Font.GothamBlack enterBtn.TextSize=16 enterBtn.ZIndex=210
    Instance.new("UICorner",enterBtn).CornerRadius=UDim.new(0,10)
    Instance.new("UIStroke",enterBtn).Color=RED

    -- Статус
    local passStatus=Instance.new("TextLabel",PassScreen)
    passStatus.Size=UDim2.new(0.8,0,0,20) passStatus.Position=UDim2.new(0.1,0,0.76,0)
    passStatus.BackgroundTransparency=1 passStatus.Text="" passStatus.TextColor3=RED
    passStatus.Font=Enum.Font.GothamBold passStatus.TextSize=13 passStatus.ZIndex=210

    -- Подпись
    local passSign=Instance.new("TextLabel",PassScreen)
    passSign.Size=UDim2.new(0.8,0,0,14) passSign.Position=UDim2.new(0.1,0,0.9,0)
    passSign.BackgroundTransparency=1 passSign.Text="@Primejtsu  •  Nazar513000  •  v5.7"
    passSign.TextColor3=Color3.fromRGB(60,60,80) passSign.Font=Enum.Font.Code passSign.TextSize=10 passSign.ZIndex=210

    -- Анимация появления
    task.wait(0.1)
    local twPass=TweenInfo.new(0.5,Enum.EasingStyle.Quart)
    TweenService:Create(keyIcon,twPass,{TextTransparency=0}):Play()
    task.wait(0.2)
    TweenService:Create(passTitle,twPass,{TextTransparency=0}):Play()
    task.wait(0.2)
    TweenService:Create(passSub,twPass,{TextTransparency=0}):Play()
    task.wait(0.2)
    TweenService:Create(inputBox,twPass,{BackgroundTransparency=0}):Play()
    TweenService:Create(enterBtn,twPass,{BackgroundTransparency=0}):Play()

    -- Пульс кнопки
    task.spawn(function()
        while enterBtn and enterBtn.Parent do
            TweenService:Create(enterBtn,TweenInfo.new(0.9,Enum.EasingStyle.Sine),{TextColor3=Color3.fromRGB(255,80,80)}):Play()
            task.wait(0.9)
            TweenService:Create(enterBtn,TweenInfo.new(0.9,Enum.EasingStyle.Sine),{TextColor3=Color3.fromRGB(140,20,20)}):Play()
            task.wait(0.9)
        end
    end)

    -- Логика проверки пароля
    local CORRECT_PASSWORD="Primejtsu"
    local attempts=0
    local passed=false

    local function tryPass()
        if passed then return end
        local input=tbox.Text
        if input==CORRECT_PASSWORD then
            passed=true
            passStatus.Text="✅  Добро пожаловать!"
            passStatus.TextColor3=GREEN
            TweenService:Create(enterBtn,TweenInfo.new(0.3),{BackgroundColor3=Color3.fromRGB(0,60,20)}):Play()
            task.wait(0.6)
            -- Красивое исчезновение
            TweenService:Create(PassScreen,TweenInfo.new(0.5,Enum.EasingStyle.Quart),{BackgroundTransparency=1}):Play()
            for _,o in ipairs(PassScreen:GetDescendants()) do
                if o:IsA("TextLabel") or o:IsA("TextButton") then
                    TweenService:Create(o,TweenInfo.new(0.4),{TextTransparency=1}):Play()
                elseif o:IsA("Frame") then
                    TweenService:Create(o,TweenInfo.new(0.4),{BackgroundTransparency=1}):Play()
                end
            end
            task.wait(0.6)
            PassScreen:Destroy()
            ...
            for i=1,6 do
                task.wait(0.04)
                inputBox.Position=origPos+UDim2.new(0,i%2==0 and 8 or -8,0,0)
            end
            inputBox.Position=origPos
            TweenService:Create(inputBox,TweenInfo.new(0.2),{BackgroundColor3=Color3.fromRGB(50,5,5)}):Play()
            task.wait(0.3)
            TweenService:Create(inputBox,TweenInfo.new(0.2),{BackgroundColor3=Color3.fromRGB(15,15,20)}):Play()
            if attempts>=5 then
                passStatus.Text="❌  Слишком много попыток!"
            else
                passStatus.Text="❌  Неверный пароль! Попытка "..attempts.."/5"
            end
            passStatus.TextColor3=RED
            tbox.Text=""
        end
    end

    enterBtn.MouseButton1Click:Connect(tryPass)
    enterBtn.TouchTap:Connect(tryPass)
    tbox.FocusLost:Connect(function(enter) if enter then tryPass() end end)
...
local function doScroll(dir)
    task.spawn(function()
        while scrolling do
            ...
            CT.CanvasPosition=Vector2.new(0,math.clamp(CT.CanvasPosition.Y+dir*28,0,maxY))
            task.wait(0.05)
        end
    end)
end
