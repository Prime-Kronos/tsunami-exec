...
-- AUTO SHOOT
-- ================================================
-- Ищем RemoteEvent стрельбы в игре Флик
local shootRemote = nil
local function findShootRemote()
    -- Ищем по типичным названиям RemoteEvent для стрельбы
    local remotes = game:GetService("ReplicatedStorage"):GetDescendants()
    local keywords = {"shoot","fire","shot","bullet","gun","snipe","attack"}
...
task.spawn(function()
    ...
    findShootRemote()
    if shootRemote then
        print("[Primejtsu X] Shoot remote found: " .. shootRemote.Name)
    else
        print("[Primejtsu X] Shoot remote not found, using mouse1click fallback")
    end
end)
...
RunService.Heartbeat:Connect(function()
    ...
    if t then
        ...
        task.spawn(function()
            if shootRemote then
                -- Стреляем через RemoteEvent — не блокирует движение совсем
                local aimPos = t.part.Position
                if shootRemote:IsA("RemoteEvent") then
                    shootRemote:FireServer(aimPos, t.part)
                elseif shootRemote:IsA("RemoteFunction") then
                    shootRemote:InvokeServer(aimPos, t.part)
                end
            elseif mouse1click then
                -- Фоллбек: mouse1click из Delta executor
                mouse1click()
            end
...
RunService.RenderStepped:Connect(function()
    ...
    -- Aimbot — плавный через Lerp но только угол, не позицию
    -- Это не блокирует движение игрока
    if Cfg.AimbotEnabled then
        local t = getTarget()
        if t then
            local newCF = CFrame.lookAt(Camera.CFrame.Position, t.part.Position)
            -- Lerp только направления взгляда, позиция остаётся от игры
            Camera.CFrame = Camera.CFrame:Lerp(newCF, Cfg.AimbotSmooth)
        end
    end
end)
...
RunService.Heartbeat:Connect(function()
    if LP.Character then
        local h=LP.Character:FindFirstChild("Humanoid")
        if h then
            -- Меняем скорость только если SpeedHack включён, иначе не трогаем
            if Cfg.SpeedEnabled then
                h.WalkSpeed = Cfg.SpeedValue
            else
                -- Восстанавливаем дефолт только если была изменена
                if h.WalkSpeed ~= 16 and not Cfg.FlyEnabled then
                    h.WalkSpeed = 16
                end
            end
