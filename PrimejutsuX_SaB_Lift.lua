...
local liftPart     = nil
local liftConn     = nil
local LIFT_SPEED   = 22  -- studs/sec вверх
...
local function startLift()
    ...
    liftConn = RunService.Heartbeat:Connect(function(dt)
        ...
        -- Держим игрока на платформе (ставим чуть выше платформы)
        local platTop = liftPart.Position.Y + (liftPart.Size.Y / 2) + 3.2
        root.CFrame   = CFrame.new(
            Vector3.new(root.Position.X, platTop, root.Position.Z)
        ) * CFrame.Angles(0, root.CFrame:ToEulerAnglesYXZ(), 0) -- сохраняем поворот
    end)
end
