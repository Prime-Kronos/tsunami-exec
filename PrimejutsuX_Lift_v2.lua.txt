...
-- ================================================
-- ПОСТОЯННАЯ ЗАЩИТА ОТ СМЕРТИ ПРИ ПОЛЁТЕ
-- Работает всегда, независимо от лифта
-- ================================================
local lastY      = nil
local fallTimer  = 0
local FALL_LIMIT = 0.4  -- секунд падения до того как включаем защиту
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if lastY then
        local falling = curY < lastY - 0.5  -- падаем вниз
        ...
        -- Если падаем дольше лимита — глушим смерть
        if fallTimer > FALL_LIMIT then
            hum:SetStateEnabled(Enum.HumanoidStateType.FallingDown, false)
            hum:SetStateEnabled(Enum.HumanoidStateType.Freefall, false)
            -- Восстанавливаем через секунду когда приземлились
        elseif not falling and fallTimer == 0 then
