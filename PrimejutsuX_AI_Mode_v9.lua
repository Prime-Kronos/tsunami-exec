...
local function deepAnalyze()
    ...
    if info.remotes and #info.remotes > 0 then
        -- Первые 20 remote
        local r = {}
        for i = 1, math.min(20, #info.remotes) do r[i] = info.remotes[i] end
        table.insert(lines, "Remotes(" .. #info.remotes .. "): " .. table.concat(r, " | "))
    end
    ...
    if info.bodyParts and #info.bodyParts > 0 then
        table.insert(lines, "BodyParts: " .. table.concat(info.bodyParts, ", "))
    end
...
local function askGroq(userMsg, callback)
    -- Каждый запрос включает свежий контекст игры
    local fullMsg = gameContext ~= "" and (gameContext .. "\nUser: " .. userMsg) or userMsg

    table.insert(history, {role="user", content=fullMsg})

    -- Держим только последние 4 сообщения чтобы не превышать лимит
    if #history > 4 then
        local h2 = {}
