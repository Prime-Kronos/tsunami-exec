-- PrimejTsuHub v5.8 | @Primejtsu | Nazar513000 | MM2
-- ФИКСЫ: Bypass v3, Trol рабочий, UI 480px без чёрного низа, Noclip v2, CoinFarm v2, Scroll Touch
...
local function isCoin(o)
    ...
    -- Строгий фильтр: только монеты, не случайные объекты
    local isCoinName=(n=="coin" or n=="dropcoin" or n=="goldcoin" or n=="silvercoin" or n=="coinpart")
    if not isCoinName then return false end
    return o.Parent~=nil and o.Transparency<0.8 and o.Parent~=LP.Character
end
...
task.spawn(function()
    while true do
        ...
        -- Берём ближайшую монету (не случайную — быстрее фарм)
        local coins={}
        ...
        repeat
            ...
            -- Обновляем направление каждые 0.4 сек
            if tick()-t0>0.4 then
                t0=tick()
                pcall(function() hum:MoveTo(target.Position) end)
            end
        until (not target or not target.Parent)
            or pcall(function() return (hrp.Position-target.Position).Magnitude < 3.5 end)
            or tick()-t0 > 5
    end
end)
...
btnUp.MouseButton1Up:Connect(stopScroll)
btnUp.MouseLeave:Connect(stopScroll)
btnUp.TouchLongPress:Connect(function() scrolling=true doScroll(-1) end)
btnUp.TouchTap:Connect(function()
    local maxY=math.max(0,CT.AbsoluteCanvasSize.Y-CT.AbsoluteSize.Y)
    CT.CanvasPosition=Vector2.new(0,math.clamp(CT.CanvasPosition.Y-90,0,maxY))
end)
...
btnDown.MouseButton1Up:Connect(stopScroll)
btnDown.MouseLeave:Connect(stopScroll)
...
mkButton("Misc","📋 Версия (уведомление)",DIM,function()
    pcall(function() game:GetService("StarterGui"):SetCore("SendNotification",{Title="Primejtsu Hub",Text="v5.8 | @Primejtsu | Nazar513000",Duration=4}) end)
end)
...
print("[Primejtsu Hub v5.8] @Primejtsu | Nazar513000 | Bypass v3 + Trol v3 + Noclip v2 + CoinFarm v2 😈🪐")
