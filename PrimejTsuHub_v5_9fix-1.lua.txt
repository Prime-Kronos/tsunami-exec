-- PrimejTsuHub v5.9 | @Primejtsu | Nazar513000 | MM2
-- ФИКСЫ: gethui(), CoinFarm v4, Noclip v2, Экран пароля
...
RunService.Heartbeat:Connect(function()
    ...
    elseif CFG.bhop then h.WalkSpeed=24
...
RunService.Stepped:Connect(function()
    ...
    elseif noclipWasOn then
        ...
        pcall(function()
            local c=getChar() if not c then return end
            for _,p in ipairs(c:GetDescendants()) do
                -- HumanoidRootPart НЕ трогаем — он вызывал застревание
...
-- ════════════════════════════════════════
task.spawn(function()
    while true do task.wait(math.random(8,18))
        if CFG.bypass then
            pcall(function()
                local vu=game:GetService("VirtualUser")
...
task.spawn(function()
    while true do task.wait(math.random(20,40))
        if CFG.bypass then
            pcall(function()
                local h=getHum() if not h then return end
                local s=h.WalkSpeed
...
task.spawn(function()
    while true do task.wait(math.random(25,55))
        if CFG.bypass and (CFG.coinFarm or CFG.bringCoins) then
            farmPaused=true task.wait(math.random(2,6)) farmPaused=false
        end
    end
end)
task.spawn(function()
    while true do task.wait(math.random(15,30))
        if CFG.bypass then pcall(function() local h=getHum() if h then h.Jump=true end end) end
    end
end)

-- ════════════════════════════════════════
-- COIN FARM v4 — МАКСИМАЛЬНЫЙ СБОР
-- ════════════════════════════════════════
...
-- Все методы сбора монеты одновременно
    ...
    pcall(function()
        if target.Parent and target.Parent~=workspace and target.Parent:IsA("Model") then
            for _,p in ipairs(target.Parent:GetDescendants()) do
                if p:IsA("BasePart") then firetouchinterest(hrp,p,0) firetouchinterest(hrp,p,1) end
            end
        end
    end)
end

-- Фоновый триггер монет рядом
task.spawn(function()
    while true do task.wait(0.12)
        ...
        pcall(function()
            for _,o in ipairs(workspace:GetDescendants()) do
                if isCoin(o) and (hrp.Position-o.Position).Magnitude<10 then
                    collectCoin(o,hrp)
                end
            end
        end)
...
task.spawn(function()
    while true do task.wait(0.05)
        ...
        for _,o in ipairs(workspace:GetDescendants()) do
            if isCoin(o) then
                pcall(function()
                    ...
                    if d<bestDist then bestDist=d target=o end
                end)
            end
        end
        ...
        -- Уже рядом
        if bestDist<5 then collectCoin(target,hrp) task.wait(0.04) continue end
        ...
        for i=1,6 do
            if not target or not target.Parent then break end
            collectCoin(target,hrp) task.wait(0.03)
        end
...
task.spawn(function()
    while true do task.wait(0.15)
        ...
        local hrp=getHRP() if not hrp then continue end
        for _,o in ipairs(workspace:GetDescendants()) do
            if isCoin(o) then pcall(function() o.CFrame=hrp.CFrame+Vector3.new(math.random(-2,2),0,math.random(-2,2)) end) end
        end
    end
end)
...
task.spawn(function()
    ...
    while task.wait(0.2) do
        ...
        for obj in pairs(prev) do
            if not obj or not obj.Parent or obj.Transparency>=0.85 then coinCount=coinCount+1 prev[obj]=nil end
        end
    end
end)
...
local function createESP(p)
    ...
    local function setup(char)
        ...
        local bb=Instance.new("BillboardGui") bb.Name="PTH57ESP" bb.AlwaysOnTop=true
        bb.Size=UDim2.new(0,130,0,72) bb.StudsOffset=Vector3.new(0,3.5,0) bb.Adornee=hrp bb.Parent=hrp bb.Enabled=false
        ...
        local nL=Instance.new("TextLabel",bb) nL.Size=UDim2.new(1,0,0,20) nL.Position=UDim2.new(0,0,0,3) nL.BackgroundTransparency=1 nL.Font=Enum.Font.GothamBlack nL.TextSize=13 nL.Text=p.Name nL.TextStrokeTransparency=0 nL.TextStrokeColor3=Color3.new(0,0,0)
        local rL=Instance.new("TextLabel",bb) rL.Size=UDim2.new(1,0,0,16) rL.Position=UDim2.new(0,0,0,24) rL.BackgroundTransparency=1 rL.Font=Enum.Font.GothamBold rL.TextSize=11 rL.TextStrokeTransparency=0 rL.TextStrokeColor3=Color3.new(0,0,0)
        local hpBg=Instance.new("Frame",bb) hpBg.Size=UDim2.new(0.88,0,0,5) hpBg.Position=UDim2.new(0.06,0,0,43) hpBg.BackgroundColor3=Color3.fromRGB(30,30,30) hpBg.BorderSizePixel=0
        ...
        local hL=Instance.new("TextLabel",bb) hL.Size=UDim2.new(1,0,0,13) hL.Position=UDim2.new(0,0,0,50) hL.BackgroundTransparency=1 hL.Font=Enum.Font.Code hL.TextSize=10 hL.TextColor3=Color3.fromRGB(200,200,200) hL.TextStrokeTransparency=0 hL.TextStrokeColor3=Color3.new(0,0,0)
        local dL=Instance.new("TextLabel",bb) dL.Size=UDim2.new(1,0,0,12) dL.Position=UDim2.new(0,0,0,62) dL.BackgroundTransparency=1 dL.Font=Enum.Font.Code dL.TextSize=10 dL.TextColor3=Color3.fromRGB(150,150,150) dL.TextStrokeTransparency=0 dL.TextStrokeColor3=Color3.new(0,0,0)
        local function upd()
...
-- Безопасный родитель для GUI (gethui для мобильных executor'ов)
...
if safeParent:FindFirstChild("PTH59") then safeParent:FindFirstChild("PTH59"):Destroy() end
local sg=Instance.new("ScreenGui")
sg.Name="PTH59" sg.ResetOnSpawn=false sg.DisplayOrder=999 sg.IgnoreGuiInset=true
...
Splash.Size=UDim2.new(1,0,1,0) Splash.BackgroundColor3=Color3.fromRGB(2,2,8) Splash.BorderSizePixel=0 Splash.ZIndex=100
...
local byLine=Instance.new("TextLabel",Splash) byLine.Size=UDim2.new(0,320,0,18) byLine.Position=UDim2.new(0.5,-160,0.7,56) byLine.BackgroundTransparency=1 byLine.Text="by  @Primejtsu  •  MM2  •  v5.9" byLine.TextColor3=Color3.fromRGB(140,110,220) byLine.Font=Enum.Font.GothamBold byLine.TextSize=13 byLine.TextTransparency=1 byLine.ZIndex=111
...
-- ЭКРАН ПАРОЛЯ
-- ════════════════════════════════════════
local function showPasswordScreen()
    local PS=Instance.new("Frame",sg)
    PS.Size=UDim2.new(1,0,1,0) PS.BackgroundColor3=Color3.fromRGB(2,2,8) PS.BorderSizePixel=0 PS.ZIndex=200

    local psBG=Instance.new("UIGradient",PS)
    psBG.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(2,2,15)),ColorSequenceKeypoint.new(1,Color3.fromRGB(5,3,20))})
    psBG.Rotation=45

    -- Фоновые звёзды
    math.randomseed(77)
    for i=1,50 do
        local s=Instance.new("Frame",PS) local sz=math.random(1,2)
        s.Size=UDim2.new(0,sz,0,sz) s.Position=UDim2.new(math.random(0,100)/100,0,math.random(0,100)/100,0)
        s.BackgroundColor3=Color3.fromRGB(180,180,255) s.BorderSizePixel=0 s.ZIndex=201
        s.BackgroundTransparency=math.random(30,70)/100
        Instance.new("UICorner",s).CornerRadius=UDim.new(1,0)
    end

    -- Иконка ключа
    local keyIco=Instance.new("TextLabel",PS)
    keyIco.Size=UDim2.new(0,70,0,70) keyIco.Position=UDim2.new(0.5,-35,0.18,-35)
    keyIco.BackgroundTransparency=1 keyIco.Text="🔑" keyIco.TextSize=56
    keyIco.Font=Enum.Font.GothamBlack keyIco.TextTransparency=1 keyIco.ZIndex=210

    -- Заголовок
    local title=Instance.new("TextLabel",PS)
    title.Size=UDim2.new(0.85,0,0,36) title.Position=UDim2.new(0.075,0,0.34,0)
    title.BackgroundTransparency=1 title.Text="ВВЕДИТЕ ПАРОЛЬ"
    title.TextColor3=RED title.Font=Enum.Font.GothamBlack title.TextSize=22 title.TextTransparency=1 title.ZIndex=210

    local sub=Instance.new("TextLabel",PS)
    sub.Size=UDim2.new(0.85,0,0,18) sub.Position=UDim2.new(0.075,0,0.47,0)
    sub.BackgroundTransparency=1 sub.Text="Primejtsu Hub  •  защита доступа"
    sub.TextColor3=Color3.fromRGB(120,120,150) sub.Font=Enum.Font.Code sub.TextSize=11 sub.TextTransparency=1 sub.ZIndex=210

    -- Поле ввода
    local inputF=Instance.new("Frame",PS)
    inputF.Size=UDim2.new(0.72,0,0,46) inputF.Position=UDim2.new(0.14,0,0.55,0)
    inputF.BackgroundColor3=Color3.fromRGB(14,14,20) inputF.BorderSizePixel=0 inputF.BackgroundTransparency=1 inputF.ZIndex=210
    Instance.new("UICorner",inputF).CornerRadius=UDim.new(0,10)
    local fStroke=Instance.new("UIStroke",inputF) fStroke.Color=Color3.fromRGB(70,70,100) fStroke.Thickness=1.5

    local tbox=Instance.new("TextBox",inputF)
    tbox.Size=UDim2.new(1,-12,1,0) tbox.Position=UDim2.new(0,6,0,0)
    tbox.BackgroundTransparency=1 tbox.BorderSizePixel=0
    tbox.PlaceholderText="пароль..." tbox.Text=""
    tbox.TextColor3=WHITE tbox.PlaceholderColor3=Color3.fromRGB(70,70,90)
    tbox.Font=Enum.Font.GothamBold tbox.TextSize=18 tbox.ClearTextOnFocus=true tbox.ZIndex=211

    -- Кнопка
    local enterBtn=Instance.new("TextButton",PS)
    enterBtn.Size=UDim2.new(0.5,0,0,42) enterBtn.Position=UDim2.new(0.25,0,0.66,0)
    enterBtn.BackgroundColor3=Color3.fromRGB(20,5,5) enterBtn.BorderSizePixel=0 enterBtn.BackgroundTransparency=1
    enterBtn.Text="ВОЙТИ" enterBtn.TextColor3=RED
    enterBtn.Font=Enum.Font.GothamBlack enterBtn.TextSize=18 enterBtn.ZIndex=210
    Instance.new("UICorner",enterBtn).CornerRadius=UDim.new(0,10)
    Instance.new("UIStroke",enterBtn).Color=RED

    -- Статус
    local statusL=Instance.new("TextLabel",PS)
    statusL.Size=UDim2.new(0.85,0,0,18) statusL.Position=UDim2.new(0.075,0,0.76,0)
    statusL.BackgroundTransparency=1 statusL.Text=""
    statusL.TextColor3=RED statusL.Font=Enum.Font.GothamBold statusL.TextSize=13 statusL.ZIndex=210

    -- Подпись
    local sign=Instance.new("TextLabel",PS)
    sign.Size=UDim2.new(0.85,0,0,14) sign.Position=UDim2.new(0.075,0,0.9,0)
    sign.BackgroundTransparency=1 sign.Text="@Primejtsu  •  Nazar513000  •  v5.9"
    sign.TextColor3=Color3.fromRGB(50,50,75) sign.Font=Enum.Font.Code sign.TextSize=10 sign.ZIndex=210

    -- Появление
    local tw=TweenInfo.new(0.45,Enum.EasingStyle.Quart)
    task.wait(0.15)
    TweenService:Create(keyIco,tw,{TextTransparency=0}):Play() task.wait(0.15)
    TweenService:Create(title,tw,{TextTransparency=0}):Play() task.wait(0.12)
    TweenService:Create(sub,tw,{TextTransparency=0}):Play() task.wait(0.15)
    TweenService:Create(inputF,tw,{BackgroundTransparency=0}):Play()
    TweenService:Create(enterBtn,tw,{BackgroundTransparency=0}):Play()

    -- Пульс кнопки
    task.spawn(function()
        while enterBtn and enterBtn.Parent do
            TweenService:Create(enterBtn,TweenInfo.new(0.85,Enum.EasingStyle.Sine),{TextColor3=Color3.fromRGB(255,70,70)}):Play() task.wait(0.85)
            TweenService:Create(enterBtn,TweenInfo.new(0.85,Enum.EasingStyle.Sine),{TextColor3=Color3.fromRGB(130,15,15)}):Play() task.wait(0.85)
        end
    end)

    -- Проверка пароля
    local PASS="Primejtsu"
    local attempts=0
    local done=false

    local function tryEnter()
        if done then return end
        if tbox.Text==PASS then
            done=true
            statusL.Text="Добро пожаловать!" statusL.TextColor3=GREEN
            TweenService:Create(fStroke,TweenInfo.new(0.3),{Color=GREEN}):Play()
            task.wait(0.7)
            TweenService:Create(PS,TweenInfo.new(0.5,Enum.EasingStyle.Quart),{BackgroundTransparency=1}):Play()
            for _,o in ipairs(PS:GetDescendants()) do
                if o:IsA("TextLabel") or o:IsA("TextButton") then TweenService:Create(o,TweenInfo.new(0.4),{TextTransparency=1}):Play()
                elseif o:IsA("Frame") then TweenService:Create(o,TweenInfo.new(0.4),{BackgroundTransparency=1}):Play() end
            end
            task.wait(0.6) PS:Destroy() showGUI()
            task.wait(0.5)
            pcall(function() game:GetService("StarterGui"):SetCore("SendNotification",{Title="Primejtsu Hub v5.9",Text="Bypass v3 | CoinFarm v4 | @Primejtsu",Duration=5}) end)
        else
            attempts=attempts+1
            -- Тряска поля
            local orig=inputF.Position
            for i=1,6 do task.wait(0.035) inputF.Position=orig+UDim2.new(0,i%2==0 and 9 or -9,0,0) end
            inputF.Position=orig
            TweenService:Create(fStroke,TweenInfo.new(0.2),{Color=RED}):Play()
            task.wait(0.4) TweenService:Create(fStroke,TweenInfo.new(0.3),{Color=Color3.fromRGB(70,70,100)}):Play()
            statusL.Text="Неверный пароль! Попытка "..attempts statusL.TextColor3=RED
            tbox.Text=""
        end
    end

    enterBtn.MouseButton1Click:Connect(tryEnter)
    tbox.FocusLost:Connect(function(enter) if enter then tryEnter() end end)
end
...
task.spawn(function()
    ...
    local steps={
        {txt="Запуск орбит...",         pct=0.15, wait=0.75},
        {txt="Загрузка звёзд...",        pct=0.30, wait=0.70},
        {txt="Инициализация ESP...",     pct=0.48, wait=0.80},
        {txt="CoinFarm v4 готов...",     pct=0.65, wait=0.75},
        {txt="Bypass v3 активирован...", pct=0.84, wait=0.72},
        {txt="Primejtsu Hub v5.9!",      pct=1.00, wait=0.70},
    }
    ...
    for _,o in ipairs(Splash:GetDescendants()) do
        ...
        elseif o:IsA("Frame") and o~=Splash then twQ(o,0.35,{BackgroundTransparency=1}) end
    end
    task.wait(0.7) Splash:Destroy()
    showPasswordScreen()
end)
...
local iconLetter=Instance.new("TextLabel",iconFrame) iconLetter.Size=UDim2.new(1,0,1,0) iconLetter.BackgroundTransparency=1 iconLetter.Text="P" iconLetter.TextColor3=Color3.new(1,1,1) iconLetter.Font=Enum.Font.GothamBlack iconLetter.TextSize=26 iconLetter.ZIndex=53
...
local lp2=Instance.new("TextLabel",Hdr) lp2.Size=UDim2.new(0,26,0,36) lp2.Position=UDim2.new(0,12,0.5,-18) lp2.BackgroundTransparency=1 lp2.Text="P" lp2.TextColor3=RED lp2.Font=Enum.Font.GothamBlack lp2.TextSize=26
...
local ls2=Instance.new("TextLabel",Hdr) ls2.Size=UDim2.new(1,-200,0,14) ls2.Position=UDim2.new(0,12,1,-16) ls2.BackgroundTransparency=1 ls2.Text="MM2 v5.9 | @Primejtsu" ls2.TextColor3=GREEN ls2.Font=Enum.Font.Code ls2.TextSize=10 ls2.TextXAlignment=Enum.TextXAlignment.Left
local xBtn=Instance.new("TextButton",Hdr) xBtn.Size=UDim2.new(0,28,0,28) xBtn.Position=UDim2.new(1,-36,0.5,-14) xBtn.BackgroundColor3=RED xBtn.Text="X" xBtn.TextColor3=WHITE xBtn.Font=Enum.Font.GothamBold xBtn.TextSize=12 xBtn.BorderSizePixel=0
...
btnUp.Size=UDim2.new(0.5,0,1,0) btnUp.BackgroundColor3=SIDE btnUp.BorderSizePixel=0
...
local function doScroll(dir)
    task.spawn(function()
        while scrolling do
            ...
            CT.CanvasPosition=Vector2.new(0,math.clamp(CT.CanvasPosition.Y+dir*32,0,maxY))
            task.wait(0.04)
        end
    end)
end
btnUp.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.Touch or i.UserInputType==Enum.UserInputType.MouseButton1 then scrolling=true doScroll(-1) end end)
btnUp.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.Touch or i.UserInputType==Enum.UserInputType.MouseButton1 then scrolling=false end end)
btnDown.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.Touch or i.UserInputType==Enum.UserInputType.MouseButton1 then scrolling=true doScroll(1) end end)
btnDown.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.Touch or i.UserInputType==Enum.UserInputType.MouseButton1 then scrolling=false end end)

-- СВАЙП пальцем
local swipeY=0 local swipeCanvas=0 local swiping=false
CT.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.Touch then swiping=true swipeY=i.Position.Y swipeCanvas=CT.CanvasPosition.Y end end)
CT.InputChanged:Connect(function(i) if i.UserInputType==Enum.UserInputType.Touch and swiping then local delta=swipeY-i.Position.Y local maxY=math.max(0,CT.AbsoluteCanvasSize.Y-CT.AbsoluteSize.Y) CT.CanvasPosition=Vector2.new(0,math.clamp(swipeCanvas+delta,0,maxY)) end end)
CT.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.Touch then swiping=false end end)

-- Layout контента
local CTL=Instance.new("UIListLayout",CT) CTL.Padding=UDim.new(0,5) CTL.SortOrder=Enum.SortOrder.LayoutOrder
local CTP=Instance.new("UIPadding",CT) CTP.PaddingLeft=UDim.new(0,8) CTP.PaddingRight=UDim.new(0,8) CTP.PaddingTop=UDim.new(0,6) CTP.PaddingBottom=UDim.new(0,6)
...
local tabIcons={Info="i",Player="R",God="S",Farm="$",Bypass="~",TP="T",Trol="!",Misc="="}
...
local ic=Instance.new("Frame") ic.Size=UDim2.new(1,0,0,88) ic.BackgroundColor3=CARD ic.BorderSizePixel=0
...
local _lp=Instance.new("TextLabel",ic) _lp.Size=UDim2.new(0,36,0,44) _lp.Position=UDim2.new(0,12,0.5,-22) _lp.BackgroundTransparency=1 _lp.Text="P" _lp.TextColor3=RED _lp.Font=Enum.Font.GothamBlack _lp.TextSize=40
...
local _n3=Instance.new("TextLabel",ic) _n3.Size=UDim2.new(1,0,0,14) _n3.Position=UDim2.new(0,12,1,-17) _n3.BackgroundTransparency=1 _n3.Text="v5.9 | CoinFarm v4 | Noclip v2 | Bypass v3" _n3.TextColor3=GREEN _n3.Font=Enum.Font.Code _n3.TextSize=10 _n3.TextXAlignment=Enum.TextXAlignment.Left
...
-- ══ GOD ══
mkSec("God","Защита")
mkToggle("God","God Mode (1e6 HP)","god",function(v) applyGod(v) end)
mkToggle("God","ESP (роли+HP)","esp")
...
mkToggle("God","Inf Ammo",nil,function(v) pcall(function() local c=getChar() if not c then return end for _,t in ipairs(c:GetChildren()) do if t:IsA("Tool") then local a=t:FindFirstChild("Ammo") if a then a.Value=v and 999 or a.Value end end end end) end)

-- ══ FARM ══
mkSec("Farm","Монеты")
mkToggle("Farm","Coin Farm v4","coinFarm")
mkToggle("Farm","Bring Coins (притянуть)","bringCoins")
...
local bypassInfo=Instance.new("Frame") bypassInfo.Size=UDim2.new(1,0,0,68) bypassInfo.BackgroundColor3=Color3.fromRGB(18,10,6) bypassInfo.BorderSizePixel=0
...
bypassTxt.Text="Bypass v3:\n- Имитация мышки (VirtualUser)\n- Паузы 2-6 сек каждые 25-55 сек\n- Случайные прыжки и смена скорости"
...
mkToggle("Bypass","Bypass v3 (включай с фармом)","bypass")
...
local function rebuildPL()
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        ...
        local rt=Instance.new("TextLabel",pf) rt.Size=UDim2.new(0,55,1,0) rt.Position=UDim2.new(1,-57,0,0) rt.BackgroundTransparency=1 rt.Text=(role=="Murderer" and "УБИЙЦА" or role=="Sheriff" and "ШЕРИФ" or "НЕВИНН") rt.Font=Enum.Font.GothamBold rt.TextSize=10 rt.TextColor3=col
...
mkToggle("Trol","Следить за жертвой",nil,function(v)
    followActive=v
    if v then task.spawn(function()
        while followActive do task.wait(0.12) pcall(function()
...
mkButton("Trol","Спам TP (4 сек)",Color3.fromRGB(60,10,10),function()
    task.spawn(function() for i=1,25 do task.wait(0.16) pcall(function()
        local hrp=getHRP() if not hrp then return end
        for _,p in ipairs(Players:GetPlayers()) do if p.Name==victimName and p.Character then
            local t=p.Character:FindFirstChild("HumanoidRootPart")
...
mkToggle("Trol","Блокировать путь жертве",nil,function(v)
    blockActive=v
    if v then task.spawn(function()
        while blockActive do task.wait(0.1) pcall(function()
            local hrp=getHRP() if not hrp then return end
            for _,p in ipairs(Players:GetPlayers()) do if p.Name==victimName and p.Character then
                local t=p.Character:FindFirstChild("HumanoidRootPart")
...
mkToggle("Trol","Мешать Убийце",nil,function(v)
    annoyActive=v
    if v then task.spawn(function()
        while annoyActive do task.wait(0.5) pcall(function()
            local hrp=getHRP() if not hrp then return end
            for _,p in ipairs(Players:GetPlayers()) do
...
mkButton("Trol","Узнать роль жертвы",Color3.fromRGB(20,30,60),function()
    for _,p in ipairs(Players:GetPlayers()) do if p.Name==victimName then
        local r=getRole(p)
        pcall(function() game:GetService("StarterGui"):SetCore("SendNotification",{Title=victimName,Text=(r=="Murderer" and "УБИЙЦА!" or r=="Sheriff" and "ШЕРИФ!" or "Невинный"),Duration=5}) end) return
    end end
end)
mkButton("Trol","Роли всех игроков",Color3.fromRGB(30,20,60),function()
    local m,s,inn={},{},{}
    for _,p in ipairs(Players:GetPlayers()) do if p~=LP then local r=getRole(p)
        if r=="Murderer" then table.insert(m,p.Name) elseif r=="Sheriff" then table.insert(s,p.Name) else table.insert(inn,p.Name) end
    end end
    local txt=""
    if #m>0 then txt=txt.."УБИЙЦА: "..table.concat(m,", ").." | " end
    if #s>0 then txt=txt.."ШЕРИФ: "..table.concat(s,", ").." | " end
    if #inn>0 then txt=txt.."НЕВИННЫЕ: "..table.concat(inn,", ") end
...
mkButton("Misc","Версия",DIM,function()
    pcall(function() game:GetService("StarterGui"):SetCore("SendNotification",{Title="Primejtsu Hub v5.9",Text="@Primejtsu | Nazar513000 | CoinFarm v4",Duration=4}) end)
end)
...
print("[PTH v5.9] @Primejtsu | Nazar513000 | gethui + CoinFarm v4 + Noclip v2 + Пароль")
