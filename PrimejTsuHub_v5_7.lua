...
-- ГЛАВНОЕ ОКНО 480px ШИРЕ + заполняет контент без чёрного низа
local W=Instance.new("Frame",sg)
W.Size=UDim2.new(0,480,0,500) W.Position=UDim2.new(0.5,-240,0.5,-250)
...
local function openGUI()
    ...
    TweenService:Create(W,TweenInfo.new(0.35,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(0,480,0,500),Position=UDim2.new(0.5,-240,0.5,-250)}):Play()
...
mkSec("Trol","Действия (все работают в MM2)")

-- 1. Следить за жертвой — ТЫ идёшь к жертве (РАБОЧИЙ)
local followActive=false
mkToggle("Trol","👣 Следить за жертвой",nil,function(v)
    followActive=v
    if v then task.spawn(function()
        while followActive do task.wait(0.15)
            pcall(function()
                local hrp=getHRP() if not hrp then return end
                for _,p in ipairs(Players:GetPlayers()) do
                    if p.Name==victimName and p.Character then
                        local t=p.Character:FindFirstChild("HumanoidRootPart")
                        if t then hrp.CFrame=t.CFrame*CFrame.new(0,0,3.5) end
                    end
                end
            end)
        end
...
-- 2. Крутить СЕБЯ (спиннер) — РАБОЧИЙ
mkButton("Trol","🌀 Крутануть себя (5 сек)",Color3.fromRGB(30,10,60),function()
    task.spawn(function()
        for i=1,60 do task.wait(0.083)
            pcall(function() local hrp=getHRP() if hrp then hrp.CFrame=hrp.CFrame*CFrame.Angles(0,math.rad(36),0) end end)
        end
    end)
end)

-- 3. Подбросить СЕБЯ — РАБОЧИЙ
mkButton("Trol","🚀 Подбросить себя",Color3.fromRGB(40,15,70),function()
    pcall(function() local hrp=getHRP() if hrp then hrp.Velocity=Vector3.new(math.random(-15,15),300,math.random(-15,15)) end end)
end)

-- 4. Заморозить СЕБЯ на месте — РАБОЧИЙ
local selfFreezeActive=false
mkToggle("Trol","🧊 Заморозить себя на месте",nil,function(v)
    selfFreezeActive=v
    if v then
        local pos=nil pcall(function() local hrp=getHRP() if hrp then pos=hrp.CFrame end end)
        task.spawn(function()
            while selfFreezeActive do task.wait(0.05)
                pcall(function() local hrp=getHRP() if hrp and pos then hrp.CFrame=pos end end)
            end
        end)
    end
end)

-- 5. Спам TP к жертве (ТЫ телепортируешься к ней) — РАБОЧИЙ
mkButton("Trol","💥 Спам TP к жертве (3 сек)",Color3.fromRGB(60,10,10),function()
    task.spawn(function()
        for i=1,18 do task.wait(0.18)
            pcall(function()
                local hrp=getHRP() if not hrp then return end
                for _,p in ipairs(Players:GetPlayers()) do
                    if p.Name==victimName and p.Character then
                        local t=p.Character:FindFirstChild("HumanoidRootPart")
                        if t then hrp.CFrame=t.CFrame*CFrame.new(math.random(-2,2),0,math.random(-2,2)) end
                    end
                end
            end)
        end
...
-- 6. Убегать от жертвы со скоростью — РАБОЧИЙ
mkButton("Trol","🏃 Убегать от жертвы (8 сек)",Color3.fromRGB(15,50,15),function()
    task.spawn(function()
        local h=getHum() if not h then return end
        local orig=h.WalkSpeed h.WalkSpeed=38
        for i=1,80 do task.wait(0.1)
            pcall(function()
                local hrp=getHRP() if not hrp then return end
                for _,p in ipairs(Players:GetPlayers()) do
                    if p.Name==victimName and p.Character then
                        local t=p.Character:FindFirstChild("HumanoidRootPart")
                        if t then local dir=(hrp.Position-t.Position).Unit h:MoveTo(hrp.Position+dir*12) end
                    end
                end
            end)
        end
...
-- 7. Выключить свет себе — РАБОЧИЙ
mkButton("Trol","🌑 Темнота (себе)",Color3.fromRGB(10,10,10),function()
    pcall(function() Lighting.Brightness=0 Lighting.ClockTime=0 Lighting.GlobalShadows=true Lighting.Ambient=Color3.fromRGB(0,0,0) Lighting.OutdoorAmbient=Color3.fromRGB(0,0,0) end)
end)
mkButton("Trol","🌟 Ультра-яркость (себе)",Color3.fromRGB(80,70,10),function()
    pcall(function() Lighting.Brightness=8 Lighting.ClockTime=14 Lighting.GlobalShadows=false Lighting.Ambient=Color3.new(1,1,1) Lighting.OutdoorAmbient=Color3.new(1,1,1) end)
end)
mkButton("Trol","🔄 Нормальный свет",Color3.fromRGB(20,40,20),function() setFB(false) end)

-- 8. TP к убийце вплотную (мешать) — РАБОЧИЙ
mkButton("Trol","⚡ TP к Убийце (мешать)",Color3.fromRGB(80,5,5),function()
    pcall(function() local hrp=getHRP() if not hrp then return end
        for _,p in ipairs(Players:GetPlayers()) do
            if p~=LP and getRole(p)=="Murderer" and p.Character then
                local t=p.Character:FindFirstChild("HumanoidRootPart")
                if t then hrp.CFrame=t.CFrame*CFrame.new(0,0,1.5) return end
            end
        end
    end)
end)

-- 9. Fake уведомления от «администратора» — РАБОЧИЙ
mkButton("Trol","📢 Fake Admin Warn (5 штук)",Color3.fromRGB(40,20,0),function()
    task.spawn(function()
        local msgs={{"⚠️ ROBLOX MODERATION","Аккаунт помечен для проверки"},{"🚨 SYSTEM ALERT","Нарушение правил обнаружено"},{"🔨 BAN NOTICE","Блокировка через 60 секунд"},{"👮 МОДЕРАТОР","Сессия под наблюдением"},{"❌ FINAL WARNING","Последнее предупреждение"}}
        for _,m in ipairs(msgs) do task.wait(1.8) pcall(function() game:GetService("StarterGui"):SetCore("SendNotification",{Title=m[1],Text=m[2],Duration=5}) end) end
    end)
end)

-- 10. Показать роли — РАБОЧИЙ
mkButton("Trol","🎭 Роли всех игроков",Color3.fromRGB(30,20,60),function()
    task.spawn(function()
        for _,p in ipairs(Players:GetPlayers()) do
            if p~=LP then task.wait(0.5)
                local r=getRole(p)
                pcall(function() game:GetService("StarterGui"):SetCore("SendNotification",{Title="👁 "..p.Name,Text=(r=="Murderer" and "🔪 УБИЙЦА" or r=="Sheriff" and "🔫 ШЕРИФ" or "😇 НЕВИННЫЙ"),Duration=4}) end)
            end
        end
    end)
end)
...
print("[Primejtsu Hub v5.7] @Primejtsu | Nazar513000 | Bypass v3 + Trol v2 😈🪐")
