...
CT.ScrollBarThickness=4 CT.ScrollBarImageColor3=RED CT.CanvasSize=UDim2.new(0,0,0,0)
CT.ScrollingDirection=Enum.ScrollingDirection.Y
CT.ElasticBehavior=Enum.ElasticBehavior.Always
-- Мобильный свайп для скролла
local touchStartY=0 local touchStartCanvas=0
CT.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.Touch then
        touchStartY=i.Position.Y
        touchStartCanvas=CT.CanvasPosition.Y
    end
end)
CT.InputChanged:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.Touch then
        local delta=touchStartY-i.Position.Y
        CT.CanvasPosition=Vector2.new(0,math.clamp(touchStartCanvas+delta,0,CT.CanvasSize.Y.Offset-CT.AbsoluteSize.Y))
    end
end)
