...
-- NOCLIP — с восстановлением коллизий при выключении
...
RunService.Stepped:Connect(function()
    ...
    elseif noclipWasOn then
        -- Восстанавливаем коллизии когда выключают
        noclipWasOn=false
        pcall(function()
            local c=getChar() if not c then return end
            ...
            -- Возвращаем персонажа наверх если упал слишком низко
            local hrp=getHRP()
...
task.spawn(function()
    while true do
        ...
        -- Бежим к монете и ждём пока не подберём (макс 4 сек)
        pcall(function() hum:MoveTo(target.Position) end)
        local t0=tick()
        repeat
            task.wait(0.05)
            if not CFG.coinFarm then break end
            -- Обновляем цель каждые 0.5 сек чтобы не застрял
            if tick()-t0>0.5 then
                pcall(function() hum:MoveTo(target.Position) end)
            end
        until (not target or not target.Parent)
            or (hrp.Position-target.Position).Magnitude < 4
            or tick()-t0 > 4
    end
end)
