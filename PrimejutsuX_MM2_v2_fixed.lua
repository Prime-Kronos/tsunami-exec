...
local function getRole(player)
    -- MM2 хранит роль в разных местах в зависимости от версии игры
    -- Проверяем все возможные места

    -- 1. Через PlayerGui (самый надёжный способ для MM2)
    local pg = player:FindFirstChild("PlayerGui")
    if pg then
        for _, v in pairs(pg:GetDescendants()) do
            if v:IsA("TextLabel") or v:IsA("TextButton") then
                local t = v.Text:lower()
                if t:find("murderer") or t == "murderer" then return "Murderer" end
                if t:find("sheriff")  or t == "sheriff"  then return "Sheriff"  end
                if t:find("hero")     or t == "hero"     then return "Hero"     end
            end
        end
    end

    -- 2. Через Character (StringValue / ObjectValue / теги)
    local char = player.Character
    if char then
        -- Ищем StringValue с ролью внутри персонажа
        for _, v in pairs(char:GetDescendants()) do
            if v:IsA("StringValue") then
                local n = v.Name:lower()
                local val = v.Value:lower()
                if n == "role" or n == "mm2role" or n == "playerrole" then
    ...
    -- 4. Через любые папки в игроке
    for _, v in pairs(player:GetDescendants()) do
        if v:IsA("StringValue") then
            local val = v.Value:lower()
            local n   = v.Name:lower()
            if (n == "role" or n:find("role")) then
                if val:find("murder") then return "Murderer" end
                if val:find("sheriff") then return "Sheriff"  end
                if val:find("hero")    then return "Hero"     end
            end
...
local function stopFarm()
    farmRunning = false
    -- Сбрасываем скорость и коллизии
    local char = LP.Character
    if not char then return end
    local h    = char:FindFirstChild("Humanoid")
    local root = char:FindFirstChild("HumanoidRootPart")
    if h and not Cfg.SpeedEnabled then
        h.WalkSpeed = 16
    end
...
local function startFarm()
    ...
    task.spawn(function()
        while farmRunning and Cfg.AutoFarm do
            local char   = LP.Character
            local myRoot = char and char:FindFirstChild("HumanoidRootPart")
            if myRoot then
                for _, v in pairs(workspace:GetDescendants()) do
                    if not Cfg.AutoFarm then break end -- прерываем если выключили
                    local n = v.Name:lower()
...
RunService.Heartbeat:Connect(function()
    ...
    if h then
        ...
        elseif not Cfg.FlyEnabled then
            h.WalkSpeed = 16
        end
    end
    if Cfg.Noclip then
    ...
    elseif lastNoclip then
        -- Нокл выключили — восстанавливаем коллизии
        lastNoclip = false
        for _, p in pairs(char:GetDescendants()) do
            if p:IsA("BasePart") then
                pcall(function() p.CanCollide = true end)
...
TabFarm:CreateToggle({
    ...
    Callback     = function(v)
        ...
        if v then
            ...
            Rayfield:Notify({
                Title   = "Auto Farm",
                Content = "Stopped. Speed and collisions reset.",
                Duration = 2,
                Image    = 4483362458,
            })
        end
