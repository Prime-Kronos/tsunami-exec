...
local function askGroq(userMsg, callback)
    ...
    -- Контекст игры идёт прямо в системный промпт — ИИ всегда его видит
    local sysWithContext = SYSTEM
    if gameContext ~= "" then
        sysWithContext = SYSTEM .. "\n\n" .. gameContext
    end

    local messages = {{role="system", content=sysWithContext}}
