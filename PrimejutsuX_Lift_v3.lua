...
-- ================================================
-- ПОСТОЯННАЯ ЗАЩИТА — блокируем резкий телепорт вниз
-- Когда несёшь брейнрот и сползаешь — игра резко
-- двигает тебя вниз на несколько studs за 1 кадр
-- Мы это детектим и возвращаем обратно
-- ================================================
local lastPos     = nil
local DROP_LIMIT  = 3.0  -- studs за 1 кадр = телепорт игры, не прыжок
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if not root or not hum then lastPos = nil; return end

    local curPos = root.Position

    if lastPos then
        local dropY = lastPos.Y - curPos.Y  -- сколько упали за кадр

        if dropY > DROP_LIMIT then
            -- Резкий дроп — возвращаем на прошлую позицию
            root.CFrame = CFrame.new(lastPos) * CFrame.Angles(0, select(2, root.CFrame:ToEulerAnglesYXZ()), 0)
            -- Обновляем цель BodyPosition если лифт активен
            if bodyPos and bodyPos.Parent then
                bodyPos.Position = lastPos
            end
            -- Сбрасываем lastPos чтобы не зациклиться
            lastPos = root.Position
            return
        end

        -- Глушим смерть от высоты всегда пока летим
        local falling = dropY > 0.3
        if falling then
